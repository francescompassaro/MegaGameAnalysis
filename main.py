...
def filter_dataframe(df: pd.DataFrame, key: str = "default") -> pd.DataFrame:
    ...
    L'utente può scegliere su quale colonna applicare il filtro.
    """
...
# --- 6. PAGINA: INSERIMENTO DATI (ADMIN) ---
    ...
    with tab_gestione_player:
        ...
        st.markdown("---")
        st.subheader("🗑️ Lista Giocatori Registrati ed Eliminazione")
        if not df_giocatori_db.empty:
            for index, row in df_giocatori_db.sort_values(by="nome").iterrows():
                p_col1, p_col2 = st.columns([5, 1])
                p_col1.write(f"👤 {row['nome']}")
                if p_col2.button("🗑️", key=f"del_player_{row['id']}"):
                    conn = sqlite3.connect(DB_FILE)
                    cursor = conn.cursor()
    ...
    with tab_gestione_deck:
        ...
        st.markdown("---")
        st.subheader("🗑️ Lista Archetipi Registrati ed Eliminazione")
            ...
            for chunk in chunks:
                ...
                for i, (idx, row) in enumerate(chunk.iterrows()):
                    with cols_deck[i]:
                        ...
                        sub_col1.write(f"🃏 {row['nome']}")
                        if sub_col2.button("🗑️", key=f"del_deck_{row['id']}"):
                            conn = sqlite3.connect(DB_FILE)
                            cursor = conn.cursor()
