...
# 3. STATO DI AUTENTICAZIONE PERSISTENTE
# Usiamo chiavi pulite isolate per evitare conflitti logici o reset lato server
if "sessione_admin_attiva" not in st.session_state:
    st.session_state["sessione_admin_attiva"] = False

if "pagina_corrente" not in st.session_state:
    st.session_state["pagina_corrente"] = "Dashboard Pubblica"

# --- BARRA LATERALE ---
st.sidebar.title("Lega Pauper Capua")

# Sezione di Login isolata
if not st.session_state["sessione_admin_attiva"]:
    st.sidebar.subheader("Accesso Admin")
    pwd_input = st.sidebar.text_input("Password", type="password", key="pwd_widget_raw")
    if st.sidebar.button("Accedi"):
        if pwd_input == PASSWORD_ADMIN:
            st.session_state["sessione_admin_attiva"] = True
...
# Elenco verticale stabili di navigazione nativi (lavorano su una singola pagina)
if st.sidebar.button("Dashboard Pubblica", use_container_width=True, type="primary" if st.session_state["pagina_corrente"] == "Dashboard Pubblica" else "secondary"):
    st.session_state["pagina_corrente"] = "Dashboard Pubblica"
    st.rerun()

if st.sidebar.button("Liste per Tappa", use_container_width=True, type="primary" if st.session_state["pagina_corrente"] == "Liste per Tappa" else "secondary"):
    st.session_state["pagina_corrente"] = "Liste per Tappa"
    st.rerun()

if st.session_state["sessione_admin_attiva"]:
    if st.sidebar.button("Inserisci Nuovi Dati", use_container_width=True, type="primary" if st.session_state["pagina_corrente"] == "Inserisci Nuovi Dati" else "secondary"):
        st.session_state["pagina_corrente"] = "Inserisci Nuovi Dati"
        st.rerun()

# --- GESTIONE ROUTING INTERNO ---
menu = st.session_state["pagina_corrente"]

# Protezione di fallback se la sessione scade ma il menu era rimasto sulla pagina admin
if menu == "Inserisci Nuovi Dati" and not st.session_state["sessione_admin_attiva"]:
    st.session_state["pagina_corrente"] = "Dashboard Pubblica"
    st.rerun()
...
# --- 6. PAGINA: INSERIMENTO DATI (ADMIN) ---
elif menu == "Inserisci Nuovi Dati" and st.session_state["sessione_admin_attiva"]:
    ...
    with tab_risultati:
        ...
        if not lista_giocatori or not lista_mazzi:
            ...
            st.markdown(f"**Calcolo Punteggio:** `{punti_totali} Punti` (3 per W, 1 per D e 2 punti presenza)")
